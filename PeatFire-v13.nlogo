...
to set-update-vulnerability
  ask patches
  [
    if water-table <= pdi ;dry
    [
      set vulnerability 1
    ]
    if water-table > pdi ;wet
    [
      set vulnerability 0
    ]
  ]
  ...
  [
    ...
    [
      ...
      [
        ...
        ; if it is already burnt below, then set status to burnt above + below, otherwise, set status to burnt above only
        ifelse burnval = 2
        [ set burnval 3 ]
        [ set burnval 1 ]
      ]
    ...
    [
      ...
      if (left-biomass-below <= 0 and burnval = 0)
      [
        set burnval 2 ;status for burnt below
      ]
      if (left-biomass-below <= 0 and burnval = 1) ;if it is already burnt above
      [
        set burnval 3 ;status for burnt above and below
      ]
    ]
...
to-report calculate-probability [wadi-direction fire-patch]
  let remaadier-degree abs (wadi-direction - fire-patch)
  ifelse remaadier-degree > 180
  [report abs (remaadier-degree - 360) / 360]
  [report remaadier-degree / 360]
end
...
;  report patches with [selection-index > 2 and distance myself <= dst and burning? = false]
  report patches with [distance myself <= dst and burning? = false]
end

to-report initial-position
;  report patches with [selection-index > 2 and burning? = false]
  report patches with [burning? = false]
end
...
to set-wtd
    ask patches
    [
      set water-table random-normal-in-bounds wtd 0.1 -1 1
      set-adi
    ]
end
